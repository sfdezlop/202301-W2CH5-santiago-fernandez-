...
export function contiguousPositionsOfAPositionY(a, b) {


  const positionRelativeModifiers = [
    { row: -1, column: -1 },
    { row: -1, column: 0 },
    { row: -1, column: 1 },
    { row: 0, column: -1 },
  ...
  for (let item = 0; item < positionRelativeModifiers.length; item++) {
    positionRelativeModifiers[item].row =
      a + positionRelativeModifiers[item].row;
    positionRelativeModifiers[item].column =
      b + positionRelativeModifiers[item].column;
    arrayTemp.push(positionRelativeModifiers[item]);
  }

  return arrayTemp;
}

function translationOfContiguousPositionsOfAPositionY(a, b) {
  return contiguousPositionsOfAPosition(a, b);
}

function contiguousPossiblePositionsOfAPositionY(a, b) {
  const arrayTemp = [];
  for (
    let i = 0;
    i < translationOfContiguousPositionsOfAPosition(a, b).length;
    i++
  ) {
    if (
      translationOfContiguousPositionsOfAPosition(a, b)[i].row > 0 &&
      translationOfContiguousPositionsOfAPosition(a, b)[i].row <=
        numberOfRowsOfTheFrame &&
      (translationOfContiguousPositionsOfAPosition(a, b)[i].column > 0) &
        (translationOfContiguousPositionsOfAPosition(a, b)[i].column <=
          numberOfColumnsOfTheFrame)
    ) {
      arrayTemp.push(
        'r' +
          translationOfContiguousPositionsOfAPosition(a, b)[i].row +
...
function contiguousPossiblePositionsOfAllPositionsButThisX(a, b) {
  const arrayTemp = contiguousPossiblePositionsOfAPosition(a, b);
  arrayTemp.shift();
  for (let i = 1; i <= numberOfRowsOfTheFrame; i++) {
    for (let j = 1; j <= numberOfRowsOfTheFrame; j++) {
      arrayTemp.push(...contiguousPossiblePositionsOfAPosition(i, j));
    }
  }
  return arrayTemp;
}
